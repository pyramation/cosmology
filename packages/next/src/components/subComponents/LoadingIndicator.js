...
const LoadingIndicator = ({
  ...
  function Loadable({ name, idx, variable, loading, loadingCondition = null }) {
    const isLoaded = loadingCondition
      ? loadingCondition()
      : variable && variable.length !== 0;

    if (isLoaded && !loadableIndicatorsDef[idx].loadedDate) {
      setLoadableIndicatorsDef(
        loadableIndicatorsDef.map((c, i) => {
          if (i === idx) {
    ...
    if (loadableIndicatorsDef[idx].loadedDate) {
      loadingTime =
        loadableIndicatorsDef[idx].loadedDate.getTime() - startLoad.getTime();
    }

    return (
      <div className="horiz" key={name}>
        <div
        ...
        <p className="detail-text" style={{ marginLeft: 16 }}>
          {variable ? variable.length : 0} items
        </p>
        <p className="detail-text" style={{ marginLeft: 16 }}>
          {loadingTime}ms
        </p>
  ...
  return (
    <div style={{ border: '1px solid #212838', padding: 8, margin: 8 }}>
      <p className="main-text">Loader status</p>
      {loadableIndicatorsConfig
        .filter((c) => c.show)
        .map((c, i) => Loadable({ ...c, idx: i, loading: loading }))}
    </div>
  );
};
