...
import { Flex, Text, Box, Icon, Divider, Tag, Button } from '@webql/core';
import { NavBarWidthWrapper } from '../../max-width-wrapper';

export const CardHeading2 = ({ title, button, lists }) => {
  return (
    <NavBarWidthWrapper py={10}>
      <Box borderRadius="md" boxShadow="0 2px 3px 0 rgba(0, 0, 0, 0.35)">
        <Flex
          ...
          py={4}
        >
          <Text fontWeight="medium" fontSize="xl" letterSpacing="wide" my={1}>
            {title}
          </Text>
          <Button my={1}>{button}</Button>
        </Flex>
        <Divider my={0} />
